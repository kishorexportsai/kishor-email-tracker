...
app.get('/auth/gmail', authMiddleware, (req, res) => {
  const oauth2Client = getOAuthClient();
  const url = oauth2Client.generateAuthUrl({ access_type: 'offline', prompt: 'consent', scope: ['https://www.googleapis.com/auth/gmail.readonly', 'https://www.googleapis.com/auth/gmail.metadata', 'email', 'profile'], state: req.user.email });
  res.redirect(url);
});
app.get('/auth/gmail/callback', async (req, res) => {
  ...
  try {
    ...
    res.send(`<!DOCTYPE html><html><body style="font-family:Arial;display:flex;align-items:center;justify-content:center;height:100vh;margin:0;background:#f0f4f8;"><div style="text-align:center;background:white;padding:40px;border-radius:12px;"><div style="font-size:60px;">✅</div><h2 style="color:#1E3A5F;">Gmail Connected!</h2><p>${gmailAddress} is now connected.</p><a href="/" style="display:inline-block;margin-top:20px;padding:12px 24px;background:#1E3A5F;color:white;text-decoration:none;border-radius:8px;">Go to Dashboard →</a></div></body></html>`);
...
async function getAccountFilter(role, account_email, email) {
  ...
  if (role === 'manager') { const { data } = await supabase.from('users').select('account_email').eq('manager_email', email); return (data || []).map(a => a.account_email).filter(Boolean); }
...
app.get('/api/emails/unreplied', authMiddleware, async (req, res) => {
  ...
  const accountFilter = await getAccountFilter(role, account_email, email);
  const { data, count } = await supabase.from('emails').select('*', { count: 'exact' }).in('account', accountFilter).eq('status', 'unreplied').order('received_at', { ascending: false }).range((page-1)*limit, page*limit-1);
...
app.get('/api/emails', authMiddleware, async (req, res) => {
  ...
  let query = supabase.from('emails').select('*', { count: 'exact' }).in('account', accountFilter).order('received_at', { ascending: false }).range((page-1)*limit, page*limit-1);
...
app.post('/api/trigger/fetch', authMiddleware, async (req, res) => { runGmailFetcher().catch(console.error); res.json({ message: 'Fetch triggered' }); });
